...
const { body: items } = await $fetch<APIResponse>($config.public.apiBase + '/api/products')

const cart = reactive(process.client ? JSON.parse(localStorage.getItem('cart') || '[]') : [])

function addToCart(id: number) {
    if(!process.client) return
    cart.push(items.find((item: any) => item.id === id))
            ...
            <div v-for="item in items" :key="item.id" class="bg-white p-4 rounded-lg border border-gray-200"
                ...
                <div class="mt-4">
                    ...
                    <div class="flex justify-between items-center mt-4">
                        ...
                        <button class="px-4 py-2 bg-emerald-500 text-white rounded hover:bg-emerald-600"
                            v-if="item.stock > 0" @click="addToCart(item.id)"
                            :disabled="cartHas(item)">Add to
                            Cart</button>
                        <p class="text-red-500" v-else>Out of Stock</p>
                    </div>
                </div>
            </div>
        </div>
